...
class CRUDStats:
    ...
    async def get_stats_by_athlete(
        self, athlete_id: str, season: Optional[str] = None
    ) -> List[StatsDocument]:
        """
    ...
    async def update_stats(
        self, stats_id: str, stats_update: StatsUpdate
    ...
    def _add_calculated_fields(self, stats_doc: StatsDocument) -> StatsDocument:
        ...
        if stats_doc.get("total_attacks", 0) > 0:
            stats_doc["attack_percentage"] = (
                StatsWithCalculations.calculate_attack_percentage(
        ...
        if stats_doc.get("total_serves", 0) > 0:
            stats_doc["serve_percentage"] = (
                StatsWithCalculations.calculate_serve_percentage(
        ...
        total_receptions = stats_doc.get("receptions", 0) + stats_doc.get(
            "reception_errors", 0
        )
        if total_receptions > 0:
            stats_doc["reception_percentage"] = (
                StatsWithCalculations.calculate_reception_percentage(
                    stats_doc.get("receptions", 0), stats_doc.get("reception_errors", 0)
                )
            )
        else:
