...
class StatsBase(BaseModel):
    ...
    season: str = Field(
        ..., min_length=1, max_length=20, description="e.g., '2025-2026'"
    )
...
class StatsWithCalculations(StatsResponse):
    ...
    @staticmethod
    def calculate_attack_percentage(
        successful: int, errors: int, total: int
    ) -> Optional[float]:
    ...
    @staticmethod
    def calculate_serve_percentage(
        aces: int, errors: int, total: int
    ) -> Optional[float]:
