...
async def create_stats(
    stats_data: StatsCreate, db: Optional[DatabaseType] = Depends(get_database)
) -> StatsResponse:
...
async def get_stats(
    stats_id: str, db: Optional[DatabaseType] = Depends(get_database)
) -> StatsResponse:
...
async def get_athlete_stats(
    ...
    season: Optional[str] = Query(
        None, description="Filter by season (e.g., '2025-2026')"
    ),
    ...
    database = _ensure_db(db)
    crud: CRUDStats = get_stats_crud(database)
    stats_list: List[StatsDocument] = await crud.get_stats_by_athlete(
        athlete_id, season
    )
